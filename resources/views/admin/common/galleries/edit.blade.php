...
@section('title')
    <title>Редактировать галерею</title>
@endsection

@section('content')
    <div class="container text-center">
        <h2 class="h2 mb-5 display-4">Редактировать галерею</h2>
        <form class="text-left w-50 m-auto" method="post" action="{{ route('admin.common.galleries.update', ['gallery' => $gallery->id]) }}" enctype="multipart/form-data">
            @csrf
            @method('put')
            <div class="form-group">
                <input value="{{ old('name', $gallery->name) }}" type="text" class="form-control @error('name') is-invalid @enderror" placeholder="Наименование" name="name">
                @error('name')
